...
$(function() {
    ...
    function load_data() {
        authorsTbl = $('#authors-tbl').DataTable({
            ...
            "pageLength": 10,
            "ajax": {
                url: "../controller/get_warehouse.php",
                method: 'POST'
            },
            columns: [
            ...
            },
            {
                ...
                // },
                {
                    ...
                    render: function(data, type, row, meta) {
                        console.log()
                        if(data == 'Admin' || data == 'Approver'){
                      
                            return '<a class="me-2  btn-sm rounded-0 py-0 edit_data" href="javascript:void(0)" data-id="' + (row.control_no) + '"><i class="ni ni-settings"></i></a>';
                        
                        }else{
                            return '<a class="me-2  btn-sm rounded-0 py-0 edit_data" href="javascript:void(0)" data-id="' + (row.control_no) + '"></a>';
            ...
            drawCallback: function(settings) {
                $('.edit_data').click(function() {
                    $.ajax({
                        url: '../controller/get_single_warehouse.php',
                        data: { control_no: $(this).attr('data-id') },
                        method: 'POST',
                        dataType: 'json',
                        error: err => {
                        ...
                        success: function(resp) {
                            if (!!resp.status) {
                                ...
                                var is_number = resp.data.is_number;
    ...
    load_data()
        ...
        $('#edit-author-frm').submit(function(e) {
            ...
            $.ajax({
                url: '../controller/update_warehouse.php',
                data: $(this).serialize(),
                method: 'POST',
                dataType: "json",
                error: err => {
