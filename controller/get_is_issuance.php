...
FROM
	...
	INNER JOIN tbl_model ON tbl_request.model_id = tbl_model.model_id
	INNER JOIN tbl_description ON tbl_request.defect_description = tbl_description.id
	INNER JOIN tbl_location ON tbl_request.defect_location = tbl_location.id  where request_status = '6' and   ({$search_where}) Group by control_no ORDER BY id ASC,  {$columns_arr[$order[0]['column']]} {$order[0]['dir']} {$limits}");
...
INNER JOIN tbl_location ON tbl_request.defect_location = tbl_location.id 
WHERE
request_status = '6'  and   ({$search_where})  Group by control_no")->num_rows;
