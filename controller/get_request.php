...
$query = $conn->query("SELECT
						...
						where request_status='1' and  ({$search_where}) group by control_no  ORDER BY id ASC ,  {$columns_arr[$order[0]['column']]} {$order[0]['dir']} {$limits} ");
...
$recordsFilterCount = $conn->query("SELECT
					--    '$role' as role1,
						...
						inner join tbl_model on tbl_request.model_id=tbl_model.model_id
						inner join tbl_description on tbl_request.defect_description=tbl_description.id
						inner join tbl_location on tbl_request.defect_location=tbl_location.id where request_status='1' and    ({$search_where})    group by control_no ")->num_rows;
