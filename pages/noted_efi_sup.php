        ...
        <div class="col-lg-12 mb-lg-0 mb-4">
          <div class="card ">
            ...
            <table  id="myTable" class="table align-items-center mb-0"  width="100%" cellspacing="0">
                ...
                <?php if($qty > 0 && $type=='admin' || $qty > 0 && $type=='Prod-EFI') {?>
...
<link href="https://cdn.datatables.net/1.13.1/js/jquery.dataTables.min.js" type='text/javascript'></script>
   ...
   $time=$datetime->format('Y-m-d H:i');
    ...
    $query = $conn->query("update  tbl_request set request_status='4', efi_sup='".$name."', date_efi_sup='".$time."'  where  control_no='" . $_GET['control_no'] . "' ");
    include('../mail/efi_sup_done.php');
    if($query){
    echo "<script>
    swal({
    ...
    window.location.href = 'dashboard_note_efi'; 
    });
    </script>"; 
    }else{
   ...
   $time=$datetime->format('Y-m-d H:i');
    ...
    $query = $conn->query("update  tbl_request set warehouse_remarks='".$remarks."',request_status='17', rejected_by='".$name."', date_rejected='".$time."'  where  control_no='" . $_GET['control_no'] . "' ");
