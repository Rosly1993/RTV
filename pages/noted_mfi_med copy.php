...
tbl_request.letter_code,tbl_request.item_code,tbl_request.reviewed_by,tbl_request.mfi_med from  tbl_request
...
<link href="https://cdn.datatables.net/1.13.1/js/jquery.dataTables.min.js" type='text/javascript'></script>
   ...
   $time=$datetime->format('Y-m-d H:i');
    ...
    $query = $conn->query("update  tbl_request set request_status='3', mfi_med='".$name."', date_mfi_med='".$time."'  where  control_no='" . $_GET['control_no'] . "' ");
    include('../mail/mfi_med_done.php');
    if($query){
    echo "<script>
    swal({
    ...
    window.location.href = 'dashboard_note'; 
    });
    </script>"; 
    }else{
   ...
   $time=$datetime->format('Y-m-d H:i');
    ...
    $query = $conn->query("update  tbl_request set request_status='9', rejected_by='".$name."', date_rejected='".$time."'  where  control_no='" . $_GET['control_no'] . "' ");
